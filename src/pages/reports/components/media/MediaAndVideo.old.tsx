 ...
 *   - loadAlerts now shows the REAL error in the status banner instead of
 *     silently falling back to mock data — so you can see exactly what's failing
 *   - Added apiError state for diagnostic display
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 9999, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}
      ...
      <div style={{ background: '#111', borderRadius: 10, overflow: 'hidden', maxWidth: '90vw', maxHeight: '90vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 80px rgba(0,0,0,0.5)' }}>
        <div style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #333' }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <a href={url} download={attachment.fileName}
              style={{ background: T.purple, color: '#fff', borderRadius: 5, padding: '4px 12px', fontSize: 11, fontWeight: 700, textDecoration: 'none' }}>
              ↓ Download
            </a>
        ...
        <div style={{ padding: '8px 16px', display: 'flex', gap: 16, alignItems: 'center' }}>
          ...
          {attachment.fileSize > 0 && <span style={{ color: '#666', fontSize: 10 }}>{(attachment.fileSize / 1024 / 1024).toFixed(2)} MB</span>}
          {attachment.duration > 0 && <span style={{ color: '#666', fontSize: 10 }}>{attachment.duration}s</span>}
        </div>
      </div>
    </div>
  );
...
const AlertDetailModal: FC<{ alert: IotAlert; onClose: () => void }> = ({ alert, onClose }) => {
  ...
  return (
    <>
      {lightboxAttachment && (
        <MediaLightbox attachment={lightboxAttachment} alertName={name} onClose={() => setLightboxAttachment(null)} />
      )}
...
const MediaGalleryTab: FC<{ alerts: IotAlert[] }> = ({ alerts }) => {
  ...
  return (
    <>
      {lightboxAttachment && (
        <MediaLightbox attachment={lightboxAttachment.att} alertName={lightboxAttachment.name} onClose={() => setLightboxAttachment(null)} />
      )}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14, padding: '16px 0' }}>
        {videoItems.map(({ att, alert }) => {
          ...
          return (
            <div key={att.id} onClick={() => setLightboxAttachment({ att, name })}
              ...
              <div style={{ padding: '8px 10px' }}>
                ...
                <div style={{ color: T.textMuted, fontSize: 10, marginTop: 2 }}>{alert.alertTime ? formatTimeAgo(alert.alertTime) : ''}</div>
              </div>
            </div>
          );
        })}
          ...
          return (
            <div key={att.id} onClick={() => setLightboxAttachment({ att, name })}
              ...
              <div style={{ padding: '6px 10px 8px' }}>
                ...
                <div style={{ color: T.textMuted, fontSize: 10, marginTop: 2 }}>CH{att.channel} · {alert.alertTime ? formatTimeAgo(alert.alertTime) : ''}</div>
              </div>
            </div>
          );
        })}
  ...
  const PAGE_SIZE = 20;
    ...
    try {
      ...
      if (res.data && res.data.length > 0) {
        ...
        // Empty result from API — show empty state
        setAlerts([]);
        setTotalPages(1);
        setTotal(0);
      }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: T.bg }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px', borderBottom: `1px solid ${T.border}`, background: T.card, flexWrap: 'wrap', gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {apiError && (
            <span style={{ background: '#fff0f0', color: '#c00', border: '1px solid #ffb3b3', borderRadius: 4, fontSize: 10, padding: '2px 7px', fontWeight: 600 }}>API Error</span>
          )}
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <button
      ...
      {/* Status banner — now shows real API errors */}
      <div style={{ background: bannerBg, borderTop: `1px solid ${bannerBdr}`, padding: '5px 20px', fontSize: 11, color: bannerColor, fontFamily: 'monospace' }}>
        {bannerText}
      </div>
    </div>
  );
