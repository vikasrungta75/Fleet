...
const Login: FC = () => {
	...
	const inputStyle = (field: string): React.CSSProperties => ({
		...
		border: `1px solid ${focusedField === field ? 'rgba(240,13,105,0.8)' : 'rgba(240,13,105,0.3)'}`,
		borderRadius: 10,
		color: '#fff',
		fontSize: 14,
		outline: 'none',
		...
		boxShadow: focusedField === field ? '0 0 0 3px rgba(240,13,105,0.15)' : 'none',
	});
	...
	return (
		<div style={{ position: 'fixed', inset: 0, width: '100vw', height: '100vh', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
			...
			<div style={{ position: 'relative', zIndex: 2, width: 440, maxWidth: '92vw', background: 'rgba(5,15,35,0.75)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', border: '1px solid rgba(240,13,105,0.25)', boxShadow: '0 0 40px rgba(240,13,105,0.18)', borderRadius: 16, padding: '36px 36px 32px' }}>
				...
				<form onSubmit={formik.handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
					...
					<button type='submit' disabled={formik.isSubmitting}
						style={{ width: '100%', padding: '13px', background: formik.isSubmitting ? 'rgba(240,13,105,0.4)' : 'linear-gradient(135deg,#f00d69,#c00050)', border: 'none', borderRadius: 10, color: '#fff', fontWeight: 700, fontSize: 15, cursor: formik.isSubmitting ? 'not-allowed' : 'pointer', marginTop: 4, letterSpacing: 0.5, boxShadow: formik.isSubmitting ? 'none' : '0 4px 20px rgba(240,13,105,0.35)' }}>
