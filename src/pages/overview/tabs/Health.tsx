...
const Health: FC<IHealthProps> = ({ dateRangeFilter, selectedKPIs, showIcons }) => {
	...
	useEffect(() => {
		...
		if (!areEqual || (maintenanceStatus.length === 0 && ecoDriverTotalScore.length === 0)) {
			dispatch.overview.getHealthMain({ startDate, endDate }); // eslint-disable-next-line react-hooks/exhaustive-deps
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [dateRangeFilter]);

	const kpiOrder = ['Maintenance Status', 'Eco Drive Average Score'];
