...
const AlertsTab: FC<{ onSelectAlert: (a: IotAlert) => void }> = ({ onSelectAlert }) => {
	...
	const alertTypes = Array.from(new Set(alerts.map(a => a.alertName || a.alarmType || a.eventType || '').filter(Boolean)));
	...
	return (
		<div>
			<FilterBar
				...
				placeholder='All Alert Types' count={alerts.length} total={total} label='Alerts'
			/>
			<div style={{ background: T.card, borderRadius: 10, border: `1px solid ${T.border}`, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
				...
				<div style={{ display: 'grid', gridTemplateColumns: '60px 110px 1fr 1fr 1fr 1fr 70px', borderBottom: `1px solid ${T.border}`, padding: '10px 20px', background: '#f8f9fa' }}>
					{['MEDIA', 'SEVERITY', 'TYPE', 'DEVICE', 'IMEI', 'TIME', 'VIEW'].map(h => (
						<div key={h} style={{ color: T.textMuted, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.8 }}>{h}</div>
					))}
				</div>

				{loading ? <Spinner /> : alerts.length === 0 ? <EmptyState icon='🔔' msg='No alerts found' /> : alerts.map((alert, i) => {
					...
					return (
						<div key={alert.id}
							style={{ display: 'grid', gridTemplateColumns: '60px 110px 1fr 1fr 1fr 1fr 70px', borderBottom: i < alerts.length - 1 ? `1px solid ${T.border}` : 'none', padding: '14px 20px', alignItems: 'center' }}
							onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.background = T.rowHover}
							onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.background = 'transparent'}>
							<div>
								{alertHasMedia(alert)
									? <span title='Has dashcam media' style={{ fontSize: 18 }}>📹</span>
